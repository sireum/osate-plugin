	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		IResourceChangeListener listener = new IResourceChangeListener() {
			public void printDelta(IResourceDelta d, String indent) {
				...
				System.out.println(indent.concat("Delta " + d.getKind() + " for resource "));
...
//			IProject currProject = SelectionHelper.getProject();
			...
			Set<AgeDiagramEditor> ade = new HashSet<AgeDiagramEditor>();
			// des.addAll(AwasUtil.getAllDiagramElements(ade.getDiagramBehavior().getAgeDiagram()));

//			Set<IProject> projects = new HashSet();
//			projects.add(currProject);
			diagramService.findDiagramsByContextBusinessObject(root).forEach(dr -> {
				ade.add(getAgeDiagramEditor(dr));
			});
			...
			if (air != null) {
				...
				server = new AwasServer(awasModel, root.getSystemInstance(), ade,
