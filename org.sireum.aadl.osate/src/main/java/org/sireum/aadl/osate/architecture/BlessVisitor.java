...
public class BlessVisitor extends BLESSSwitch<Boolean> {
	...
	private Option<Classifier> resolveBlessType(TypeDeclaration t) {
		...
		for (Entry<String, Component> e : v.datamap.entrySet()) {
			List<Property> props = VisitorUtil.isz2IList(e.getValue().properties());
			for(Property p : props) {
				String propertyName = p.name().name().apply(VisitorUtil.toZ(0)).string();
				if(propertyName.equalsIgnoreCase("BLESS::Typed")) {
					assert(p.getPropertyValues().size().toInt() == 1);
					ValueProp value = (ValueProp) p.getPropertyValues().apply(VisitorUtil.toZ(0));
	...
	public Boolean caseValue(Value object) {
		...
		if (object.getConstant() != null) {
			...
			if (classifier.nonEmpty()) {
				...
				BTSAccessExp ae = BTSAccessExp$.MODULE$.apply(t, enumValue);
				push(ae);
			} else {
	...
	public Boolean caseValueName(ValueName object) {
		...
		BTSExp ret = null;
		BTSExp objectName = BTSNameExp$.MODULE$.apply(n);

		if (object.isDot()) {
			List<PartialName> attributeIds = object.getPn();

			ret = BTSAccessExp$.MODULE$.apply(objectName, attributeIds.get(0).getRecord_id());
			for (int i = 1; i < attributeIds.size(); i++) {
				ret = BTSAccessExp$.MODULE$.apply(ret, attributeIds.get(i).getRecord_id());
			}
	...
	public Boolean caseDisjunction(Disjunction object) {
		...
		BTSBinaryExp be = BTSBinaryExp$.MODULE$.apply(op, lhs, rhs);
		push(be);
